...
from userbot.utils import VAMPBOT_cmd, sudo_cmd, edit_or_reply
...
@borg.on(VAMPBOT_cmd(outgoing=True, pattern="clock$"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("🕙🕘🕗🕖🕕🕔🕓🕒🕑🕐🕛"))
...
@borg.on(VAMPBOT_cmd(pattern=r"muah$"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("😗😙😚😚😘"))
...
@borg.on(VAMPBOT_cmd(pattern="heart$"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("❤️🧡💛💚💙💜🖤"))
...
@borg.on(VAMPBOT_cmd(pattern="gym$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("🏃‍🏋‍🤸‍🏃‍🏋‍🤸‍🏃‍🏋‍🤸‍"))
...
@borg.on(VAMPBOT_cmd(pattern=f"earth$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("🌏🌍🌎🌎🌍🌏🌍🌎"))
...
@borg.on(VAMPBOT_cmd(outgoing=True, pattern="moon$"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("🌗🌘🌑🌒🌓🌔🌕🌖"))
...
@borg.on(VAMPBOT_cmd(pattern=f"smoon$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(pattern=f"tmoon$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 0.1
...
@borg.on(VAMPBOT_cmd(pattern=f"clown$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(pattern=f"human$", outgoing=True))
...
@borg.on(VAMPBOT_cmd(pattern=f"music$", outgoing=True))
...
@borg.on(VAMPBOT_cmd(pattern=f"squ$", outgoing=True))
...
@borg.on(VAMPBOT_cmd(outgoing=True, pattern="kiler( (.*)|$)"))
