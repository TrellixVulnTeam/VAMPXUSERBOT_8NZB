...
@borg.on(VAMPBOT_cmd(pattern="gdrive ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(pattern="gclear", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
...
@borg.on(VAMPBOT_cmd(pattern="gdir ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
...
@borg.on(VAMPBOT_cmd(pattern="drive (delete|get) ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
...
@borg.on(VAMPBOT_cmd(pattern="sdrive ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
...
async def create_token_file(token_file, event):
    ...
    async with event.client.conversation(Var.PLUGIN_CHANNEL) as conv:
        await conv.send_message(
            f"Go to the following link in your browser: {authorize_url} and reply the code"
        )
        response = conv.wait_event(
            events.NewMessage(outgoing=True, chats=Var.PLUGIN_CHANNEL)
        )
