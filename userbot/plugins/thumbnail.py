...
@borg.on(VAMPBOT_cmd(pattern="savethumbnail"))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(pattern="clearthumbnail"))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(pattern="getthumbnail"))
async def _(event):
    if event.fwd_from:
        return
    if event.reply_to_msg_id:
        ...
        try:
            a = await borg.download_media(
                r.media.document.thumbs[0], Var.TEMP_DOWNLOAD_DIRECTORY
            )
