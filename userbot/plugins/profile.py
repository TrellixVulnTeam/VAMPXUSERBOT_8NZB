...
@borg.on(VAMPBOT_cmd(pattern="pbio (.*)"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(pattern="pname ((.|\n)*)"))  # pylint:disable=E0602,W0703
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(pattern="ppic"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(outgoing=True, pattern="username (.*)"))
...
@borg.on(VAMPBOT_cmd(outgoing=True, pattern="count$"))
...
@borg.on(VAMPBOT_cmd(outgoing=True, pattern=r"delpfp"))
...
@borg.on(VAMPBOT_cmd(pattern="myusernames$"))
async def _(event):
    if event.fwd_from:
        return
