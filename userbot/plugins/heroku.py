...
@borg.on(VAMPBOT_cmd(pattern="(logs|log)"))
...
@borg.on(VAMPBOT_cmd(pattern="(rerun|restarts)"))
...
@borg.on(VAMPBOT_cmd(pattern="usage$"))
...
@borg.on(
    VAMPBOT_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", outgoing=True)
)
@borg.on(
...
@borg.on(VAMPBOT_cmd(pattern="shp ?(.*)"))
async def lel(event):
...
@borg.on(VAMPBOT_cmd(pattern="acolb (.*)"))
async def sf(event):
...
@borg.on(VAMPBOT_cmd(pattern="tfa (.*)"))
async def l(event):
    hmm = event.pattern_match.group(1)
    app = Heroku.app(Var.HEROKU_APP_NAME)
