...
from userbot.utils import VAMPBOT_cmd, humanbytes, progress, register, time_formatter
...
@borg.on(VAMPBOT_cmd(pattern="download(?: |$)(.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
# @borg.on(VAMPBOT_cmd(pattern="uploadir (.*)", allow_sudo=True))
...
# @borg.on(VAMPBOT_cmd(pattern="upload (.*)", allow_sudo=True)
# @register(pattern=r".upload (.*)", outgoing=True)
@borg.on(VAMPBOT_cmd(pattern="upload (.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
...
# @borg.on(VAMPBOT_cmd(pattern="uploadas(stream|vn|all) (.*)", allow_sudo=True))
