...
@borg.on(VAMPBOT_cmd(pattern="userinfo(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(pattern="whois(?: |$)(.*)"))
async def who(event):
...
async def fetch_info(replied_user, event):
    ...
    caption = "<b>USER INFO from VAMPUSERBOT's database :</b>\n\n"
...
@borg.on(VAMPBOT_cmd(pattern="link(?: |$)(.*)"))
