    ...
    while SPOTIFYCHECK:
        try:
            ...
            if errorcheck == 0:
                ...
                print(ERROR_MSG)
                if Var.LOGGER:
                    await borg.send_message(Var.PM_LOGGR_BOT_API_ID, ERROR_MSG)
...
@borg.on(VAMPBOT_cmd(pattern=f"ensp", allow_sudo=True))
...
@borg.on(VAMPBOT_cmd(pattern=f"disp", allow_sudo=True))
