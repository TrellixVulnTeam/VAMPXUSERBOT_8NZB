...
@borg.on(VAMPBOT_cmd(pattern=r"star$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("🦋✨🦋✨🦋✨🦋✨"))
...
@borg.on(VAMPBOT_cmd(pattern=r"boxs"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("🟥🟧🟨🟩🟦🟪🟫⬛⬜"))
...
@borg.on(VAMPBOT_cmd(pattern=f"rain$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("🌬☁️🌩🌨🌧🌦🌥⛅🌤"))
...
@borg.on(VAMPBOT_cmd(pattern=r"clol$"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("🤔🧐🤨🤔🧐🤨"))
...
@borg.on(VAMPBOT_cmd(pattern=r"odra$"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("🚶🏃🚶🏃🚶🏃🚶🏃"))
...
@borg.on(VAMPBOT_cmd(pattern="dump ?(.*)"))
async def _(message):
    try:
...
@borg.on(VAMPBOT_cmd(pattern="fleaveme$"))
async def _(event):
...
@borg.on(VAMPBOT_cmd(pattern="loveu", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(VAMPBOT_cmd(pattern=f"plane", outgoing=True))
async def _(event):
    if event.fwd_from:
        retun
    await event.edit("✈-------------")
...
@borg.on(VAMPBOT_cmd(pattern=f"jio$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 1
...
@borg.on(VAMPBOT_cmd(pattern=f"solarsystem", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
