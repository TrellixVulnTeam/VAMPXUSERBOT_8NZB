...
DEFAULTUSERBIO = str(BIO_MSG) if BIO_MSG else "VAMPUSERBOT"
DEL_TIME_OUT = 60
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "VAMPUSERBOT"


@bot.on(VAMPBOT_cmd(pattern="autoname"))  # pylint:disable=E0602
async def _(event):
...
@bot.on(VAMPBOT_cmd(pattern="autobio"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    while True:
